...
namespace GridSystem {
    public class Grid<TGridNode> : IGrid<TGridNode> where TGridNode : IGridNode<TGridNode> {
        ...
        public Grid(int width, int height, Func<Grid<TGridNode>, int, int, TGridNode> createGridNode) : this(width,
            height, 1f, Vector3.zero, new Vector2(0.5f, 0.5f), createGridNode) { }
        ...
        public void Dispose() {
            for (int x = 0; x < Width; x++) {
                for (int y = 0; y < Height; y++) {
                    if (_gridArray[x, y] == null) continue;
                    _gridArray[x, y].OnGridNodeChanged -= OnGridNodeChangedEventPropagation;
            ...
            OnGridNodeChanged = null;
            OnGridNodeAdded = null;
            OnGridNodeRemoved = null;
        }

        private void OnGridNodeChangedEventPropagation(object sender, GridNodeChangedEvent args) {
        ...
        protected void TriggerGridNodeAdded(int x, int y) {
            Debug.Assert(x >= 0 && x < Width && y >= 0 && y < Height,
                $"{nameof(TriggerGridNodeAdded)}: ({x}, {y}) is out of bounds.");
            OnGridNodeAdded?.Invoke(this, new GridNodeAddedEvent(x, y));
        }

        protected void TriggerGridNodeRemoved(int x, int y) {
            Debug.Assert(x >= 0 && x < Width && y >= 0 && y < Height,
                $"{nameof(TriggerGridNodeRemoved)}: ({x}, {y}) is out of bounds.");
            OnGridNodeRemoved?.Invoke(this, new GridNodeRemovedEvent(x, y));
        }
        ...
        public List<TGridNode> GetGridNodesInArea(int x, int y, int width, int height) {
            ...
            for (var iY = 0; iY < height; iY++) {
                for (var iX = 0; iX < width; iX++) {
                    TGridNode node = GetGridNode(x + iX, y + iY);
                    if (node != null) {
                        nodes.Add(node);
                    }
                }
        ...
        public void SetGridNodesInArea(int x, int y, int width, int height, Func<int, int, TGridNode> nodeFactory) {
            for (int iY = 0; iY < height; iY++) {
                for (int iX = 0; iX < width; iX++) {
                    var node = nodeFactory != null ? nodeFactory(x + iX, y + iY) : default;
                    SetGridNode(x + iX, y + iY, node);
                }
            }
        }
